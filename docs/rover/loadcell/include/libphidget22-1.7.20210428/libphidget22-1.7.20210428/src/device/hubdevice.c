...
PhidgetHubDevice_initAfterOpen(PhidgetDeviceHandle device) {
	...
	case PHIDUID_HUB0001:
		...
		assert(len == (phid->devChannelCnts.numVintPorts + 1));
...
readInTXBufferCounts(PhidgetHubDeviceHandle phid, int port) {
	...
	case PHIDUID_HUB0001:
		len = sizeof(buffer);
		ret = PhidgetDevice_transferpacket(NULL, (PhidgetDeviceHandle)phid, PHIDGETUSB_REQ_DEVICE_READ,
			(VINTHUB_PACKET_HUB | VINTHUB_HUBPACKET_GETTXBUFFERSTATUS), 0, buffer, &len, 100);

		if (ret != EPHIDGET_OK) {
			logerr("Error reading VINTHUB_HUBPACKET_GETTXBUFFERSTATUS after a packet loss.");
			return;
		}
...
PhidgetHubDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case BP_SETPORTPOWER:
		...
		return sendHubPortPacket(bp->iop, phid, hubPort, VINTHUB_HUBPACKET_PORTPOWER, buffer, 1);
...
PhidgetHubDevice_setPortMode(mosiop_t iop, PhidgetHubDeviceHandle phid, int index, PhidgetHub_PortMode newVal) {
	...
	return sendHubPortPacket(iop, phid, index, VINTHUB_HUBPACKET_SETPORTMODE, buffer, 1);
}
...
_setADCCalibrationValues(mosiop_t iop, PhidgetHubDeviceHandle phid,
	...
	case PHIDUID_HUB0001:
	{
		...
		pack32(buffer, 0x8ef90234);
		bufferlen = 4;
		ret = PhidgetDevice_transferpacket(NULL, (PhidgetDeviceHandle)phid, PHIDGETUSB_REQ_CHANNEL_WRITE,
			VINT_PACKET_TYPE_CALIBRATION_MODE, 0, buffer, &bufferlen, DEFAULT_TRANSFER_TIMEOUT);
		if (ret != EPHIDGET_OK)
			return (ret);
		...
		bufferlen = 24;
		ret = PhidgetDevice_transferpacket(NULL, (PhidgetDeviceHandle)phid, PHIDGETUSB_REQ_CHANNEL_WRITE,
			VINT_PACKET_TYPE_CALIBRATION_WRITE, 0, buffer, &bufferlen, DEFAULT_TRANSFER_TIMEOUT);
		if (ret != EPHIDGET_OK)
			return (ret);

		bufferlen = 0;
		ret = PhidgetDevice_transferpacket(NULL, (PhidgetDeviceHandle)phid, PHIDGETUSB_REQ_CHANNEL_WRITE,
			VINT_PACKET_TYPE_CALIBRATION_EXIT, 0, buffer, &bufferlen, DEFAULT_TRANSFER_TIMEOUT);
