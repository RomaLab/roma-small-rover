...
typedef enum RXErrorCode {
	RX_ERROR_OK = 0,		//No error occurred
	RX_ERROR_UNKNOWN = 1,	//No code specified
	RX_ERROR_TIMEOUT = 2,	//RX timed out while waiting
	RX_ERROR_FORMAT = 3,	//A protocol-specific error occurred, e.g. parity error. Likely noise on the line
	RX_ERROR_INVALID = 4,	//The input lines have been detected as INVALID
	RX_ERROR_OVERRUN = 5,	//Data is being received on RX faster than it can be dealt with
	RX_ERROR_CORRUPT = 6,	//Somehow the incoming data got out of sequence
} RXErrorCode;
