...
int start(int argc, char **argv)
{
    ...
    SetWheelSpeed setSpeed;
    Brake brake;
    WheelInitialize initialize;
    TorqueOff off;
    initialize.controller = new MultiMotorController(dxl_wb,yaml_file,Velocity);

    while (ros::ok())
    {
        char c;
        ROS_INFO("s=SetSpeed, b=Brake, i=WheelInitialize, q=Quit");
        cin >> c;
        
        switch (c)
        {
        case 's':
            ROS_INFO("Enter your goal velocity for wheel motor");
            for (size_t i = 0; i < 4; i++)
            {
                // ROS_INFO("Motor %d: ",int(i));
                setSpeed.goalSpeed[i]=100;
            }
            send_event(setSpeed);
            break;
        case 'b':
